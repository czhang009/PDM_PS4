...
# line plot of employment status (colored by sex) across time
plot <- ipums_rec %>%
  group_by(year, sex_rec, empstat_rec) %>%
  summarise(count = n(), .groups = 'drop') %>%
  group_by(year, sex_rec) %>%
  mutate(prop = count/sum(count))
# if using this one, color by pink/blue
...
ggplot(plot2, aes(x = marst_rec, y = prop, fill = empstat_rec)) +
  ...
  "Employed" = "#C3B1E1",     # pastel blue
  "Unemployed" = "#A1C9F4",   # pastel orange
  "Not in labor force" = "#8DE5A1"  # pastel green
))
```
